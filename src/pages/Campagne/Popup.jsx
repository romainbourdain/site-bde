...
const Popup = ({ id, setShow }) => {
  ...
  const data = listeData[id - 1];

  if (id < 1 || id > 4) {
    console.error("erreur id popup");
    setShow(-1);
  }
        ...
        <Window>
          ...
          <SocialNetworks>
            {data.website_link && (
              <Icon
                icon={faGlobe}
                onClick={() => window.open(data.website_link)}
              />
            )}
            {data.email && (
              <Icon
            ...
            {data.facebook_link && (
              <Icon
                icon={faFacebook}
                onClick={() => window.open(data.facebook_link)}
              />
            )}
            {data.instagram_link && (
              <Icon
                icon={faInstagram}
                onClick={() => window.open(data.instagram_link)}
              />
            )}
            {data.twitter_link && (
              <Icon
            ...
            {data.discord_link && (
              <Icon
                icon={faDiscord}
                onClick={() => window.open(data.discord_link)}
              />
            )}
          </SocialNetworks>
